...
template = env.get_template('mj_invoice_template.html')
...
for order_number, group in grouped_orders:
    ...
    for _, line_item_row in group.iterrows():
        ...
        # Calculate pricelist (extended price before discount applied to net price)
        pricelist = net_price  # The base price per unit
    ...
    output_html_filename = f"mj_invoice_{order_number}.html"
    output_pdf_filename = f"mj_invoice_{order_number}.pdf"
    ...
    print(f"Generated M&J Toys invoice for order number {order_number} saved as {output_pdf_filename}")

print("All M&J Toys invoices have been processed.")
